...
// Lazy: the pierre-rendered code view (FilePreviewCode.tsx) drags the full
// `@pierre/diffs` + Shiki closure, which must stay out of this module's
// static import graph — FilePreview is reachable from the workspace route
// chunk that gates every session's first paint.
const FilePreviewCode = lazy(() =>
  import("@/components/git-diff/diff-islands").then((module) => ({
    default: module.FilePreviewCode,
  })),
);
